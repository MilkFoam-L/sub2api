...
func TestExportDataPassesAccountFiltersAndSort(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodGet,
		"/api/v1/admin/accounts/data?platform=openai&type=oauth&status=active&group=12&privacy_mode=blocked&openai_plan_type=plus&search=keyword&sort_by=priority&sort_order=desc",
		nil,
	)
	router.ServeHTTP(rec, req)
	require.Equal(t, http.StatusOK, rec.Code)
	...
	require.Equal(t, "plus", adminSvc.lastListAccounts.openAIPlanType)
...
func TestImportDataBindsSelectedGroupsWhenDefaultGroupSkipped(t *testing.T) {
	router, adminSvc := setupAccountDataRouter()
	...
	require.Len(t, adminSvc.createdAccounts, 1)
	require.True(t, adminSvc.createdAccounts[0].SkipDefaultGroupBind)
	require.Equal(t, []int64{10, 20}, adminSvc.createdAccounts[0].GroupIDs)
