...
func (r *accountRepository) accountListFilteredQuery(platform, accountType, status, search string, groupID int64, privacyMode, openAIPlanType string) *dbent.AccountQuery {
	...
	if platform == service.PlatformOpenAI && openAIPlanType != "" {
		q = q.Where(dbpredicate.Account(func(s *entsql.Selector) {
			s.Where(sqljson.ValueEQ(dbaccount.FieldCredentials, openAIPlanType, sqljson.Path("plan_type")))
		}))
	}
...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode, openAIPlanType string) ([]service.Account, *pagination.PaginationResult, error) {
	q := r.accountListFilteredQuery(platform, accountType, status, search, groupID, privacyMode, openAIPlanType)
