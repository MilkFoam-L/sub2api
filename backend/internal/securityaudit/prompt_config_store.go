...
func (m *ConfigManager) buildNextStorage(current storageConfig, req UpdateConfigRequest, actorID int64) (storageConfig, error) {
	...
	next := storageConfig{
		...
		StoreFullPrompt: req.StoreFullPrompt, Strategy: strings.TrimSpace(req.Strategy), WorkerCount: req.WorkerCount,
	...
	for _, endpoint := range req.Endpoints {
		...
		case hadOld && old.BaseURL == stored.BaseURL:
			stored.TokenCiphertext = old.TokenCiphertext
		case hadOld && old.TokenCiphertext != "":
			return storageConfig{}, infraerrors.BadRequest(
				"prompt_audit_token_required_for_url_change",
				"修改审计节点地址时必须重新提供 API Key，或显式清除旧 API Key",
			)
		}
