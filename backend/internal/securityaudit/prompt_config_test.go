...
func TestDefaultConfigIsOff(t *testing.T) {
	...
	require.False(t, storage.StoreFullPrompt)
	...
	require.Contains(t, string(publicJSON), `"store_full_prompt":false`)
}
...
func TestBuildNextStoragePreserveReplaceAndClearToken(t *testing.T) {
	...
	changedURLReq := base
	changedURLReq.Endpoints = append([]UpdateEndpoint(nil), base.Endpoints...)
	changedURLReq.Endpoints[0].BaseURL = "http://127.0.0.1:9090"
	_, err = manager.buildNextStorage(current, changedURLReq, 9)
	require.Error(t, err)
	require.Equal(t, "prompt_audit_token_required_for_url_change", infraerrors.Reason(err))

	changedURLReq.Endpoints[0].Token = "replacement"
	changed, err := manager.buildNextStorage(current, changedURLReq, 9)
	require.NoError(t, err)
	require.Equal(t, "enc:replacement", changed.Endpoints[0].TokenCiphertext)
}
