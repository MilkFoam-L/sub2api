...
func TestSnapshotFullPromptKeepsUnredactedText(t *testing.T) {
	...
	snapshot, err := ExtractPromptSnapshot(Request{Protocol: "openai_chat_completions", Body: []byte(body)})
	require.NoError(t, err)
	// Extraction keeps the full prompt transiently for scanning. Persistence is
	// independently opt-in and defaults to withholding the sensitive text.
	...
	require.Empty(t, snapshot.ForStorage(true).ScanText)
}
