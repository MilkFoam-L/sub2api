...
func TestPromptAuditSensitiveRoutesRequireStepUp(t *testing.T) {
	...
	for _, route := range []struct {
		...
		{http.MethodPut, "/api/v1/admin/prompt-audit/config"},
		{http.MethodPost, "/api/v1/admin/prompt-audit/endpoints/probe"},
		{http.MethodGet, "/api/v1/admin/prompt-audit/events/1"},
		{http.MethodDelete, "/api/v1/admin/prompt-audit/events/1"},
		{http.MethodPost, "/api/v1/admin/prompt-audit/events/batch-delete"},
		{http.MethodPost, "/api/v1/admin/prompt-audit/events/delete-by-filter"},
	} {
		recorder := httptest.NewRecorder()
		request := httptest.NewRequest(route.method, route.path, strings.NewReader(`{}`))
		request.Header.Set("Content-Type", "application/json")
		router.ServeHTTP(recorder, request)
		require.Equalf(t, http.StatusPreconditionRequired, recorder.Code, "%s %s", route.method, route.path)
	}
}
