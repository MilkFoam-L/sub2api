...
const apiKeyAuthSnapshotVersion = 14 // v14: include user privacy filter opt-in flag, group peak rate fields, and group video pricing fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		User: APIKeyAuthUserSnapshot{
			...
			PrivacyFilterEnabled:       apiKey.User.PrivacyFilterEnabled,
		},
	}
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		User: &User{
			...
			PrivacyFilterEnabled:       snapshot.User.PrivacyFilterEnabled,
