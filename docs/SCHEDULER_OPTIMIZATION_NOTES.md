...
本轮已把调度策略从“系统设置 -> 网关”迁移到独立的管理员“调度面板”，侧边栏入口位于“账号管理”下方。系统设置页不再展示调度策略卡片，也不再在保存其他系统设置时提交 `gateway_scheduling`，避免误覆盖调度配置。

### 调度面板能力

- 支持配置评分权重、阈值、粘性会话、主动探活暂停和恢复慢启动。
- 支持按分组选择优先调度账号，未配置的分组使用常规调度；旧的全局 `preferred_account_id` 仅保留兼容。
- 展示当前调度顺序说明：硬过滤 -> 粘性会话 -> priority 分层 -> 同层优先账号 -> Weighted P2C 成本评分。
- 展示最近内存调度日志，便于观察实际账号选择结果。

### 优先账号边界

优先账号只影响当前请求分组下、已通过硬过滤、匹配当前模型的候选账号：

- 按 `preferred_account_by_group_id` 区分不同分组，未配置的分组不继承其他分组的优先账号。
- 不绕过禁用、限流、过载、临时暂停、额度、RPM、模型能力等硬过滤。
- 不修改账号自身 `priority` 字段。
- 不跨越 priority 分层，只在当前候选层内临时置顶。
- 粘性会话仍优先于优先账号，避免破坏已有会话上下文。

### 调度日志边界

- 日志为进程内存环形日志，服务重启后清空。
- 默认保留最近 200 条，接口最多返回 200 条。
- 只记录成功选择、粘性命中/重绑、无候选、无可用槽位等关键结果。
- 不逐账号记录过滤过程，避免高并发放大开销。
- 不记录请求正文、密钥、Token 等敏感信息。

### 独立接口

- `GET /api/v1/admin/scheduling/config`：读取调度配置。
- `PUT /api/v1/admin/scheduling/config`：保存调度配置与优先账号，只更新调度相关 setting。
