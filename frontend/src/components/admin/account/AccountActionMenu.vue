      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isOpenAIOAuthParent" @click="$emit('toggle-openai-team-401-retryable', account, !isOpenAITeam401Retryable); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="sync" size="sm" />
              {{ t(isOpenAITeam401Retryable ? 'admin.accounts.disableOpenAITeam401Retryable' : 'admin.accounts.enableOpenAITeam401Retryable') }}
            </button>
            ...
            <button v-if="hasUpstreamNoBalance" @click="$emit('recover-upstream-balance', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="check" size="sm" />
              {{ t('admin.accounts.recoverUpstreamBalance') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'recover-upstream-balance', 'reset-quota', 'set-privacy', 'create-spark-shadow', 'toggle-openai-team-401-retryable'])
const { t } = useI18n()
...
const hasUpstreamNoBalance = computed(() => {
  if (props.account?.status !== 'error' || !props.account.error_message) return false
...
const isOpenAITeam401Retryable = computed(() => props.account?.credentials?.openai_team_401_retryable === true)
