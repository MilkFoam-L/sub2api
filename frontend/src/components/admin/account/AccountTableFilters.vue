<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select
      v-if="filters.platform === 'openai'"
      :model-value="filters.openai_plan_type"
      class="w-40"
      :options="openAIPlanOpts"
      @update:model-value="updateOpenAIPlanType"
      @change="$emit('change')"
    />
...
const updatePlatform = (value: string | number | boolean | null) => {
  ...
  emit('update:filters', {
    ...props.filters,
    platform,
    openai_plan_type: platform === 'openai' ? props.filters.openai_plan_type || '' : ''
  })
}
...
const updateOpenAIPlanType = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, openai_plan_type: value }) }
...
const openAIPlanOpts = computed(() => [
  { value: '', label: t('admin.accounts.openAIPlanTypes.all') },
