<template>
  ...
  >
    ...
    <div class="sidebar-header" :class="{ 'sidebar-header-collapsed': sidebarCollapsed }">
      ...
      <router-link
        :to="homePath"
        class="sidebar-logo flex h-9 w-9 items-center justify-center overflow-hidden rounded-md border border-border bg-background shadow-sm transition-opacity hover:opacity-80"
      ...
      <div class="sidebar-brand" :class="{ 'sidebar-brand-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
        <router-link
          :to="homePath"
          class="sidebar-brand-title text-lg font-semibold text-foreground transition-colors hover:text-primary"
          @click="handleMenuItemClick(homePath)"
        >
          {{ siteName }}
        </router-link>
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/scheduling', label: t('nav.scheduling'), icon: SignalIcon },
