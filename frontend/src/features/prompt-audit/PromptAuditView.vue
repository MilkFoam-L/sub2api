    ...
    <div v-if="draft && activeTab === 'config'" class="fixed inset-x-0 bottom-0 z-30 border-t border-gray-200 bg-white/95 px-4 py-3 shadow-[0_-12px_35px_rgba(15,23,42,0.08)] backdrop-blur dark:border-dark-700/80 dark:bg-dark-900/95 dark:shadow-[0_-12px_35px_rgba(0,0,0,0.35)] lg:left-64">
      <div class="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3">
        <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <SaveToggle :label="t('admin.promptAudit.saveBar.storeFull')" :model-value="draft.store_full_prompt" data-test="store-full-toggle" @update:model-value="replaceDraft({ ...draft!, store_full_prompt: $event })" />
        </div>
...
async function runProbe(endpoint: PromptAuditEndpointDraft) {
  ...
  try {
    ...
    if (!isStepUpCancelled(error)) appStore.showError(errorMessage(error, 'admin.promptAudit.errors.probe'))
  } finally {
...
async function openEvent(id: number) {
  ...
  try { activeEvent.value = await promptAuditStepUp.run(() => promptAuditAPI.getEvent(id)) }
    ...
    if (!isStepUpCancelled(error)) appStore.showError(errorMessage(error, 'admin.promptAudit.errors.loadDetail'))
...
async function confirmIDDelete() {
  ...
  try {
    ...
    if (!isStepUpCancelled(error)) appStore.showError(errorMessage(error, 'admin.promptAudit.errors.delete'))
  }
...
async function confirmFilterDelete() {
  if (loading.deleting || !deletePreview.value || !deletePreviewFilters.value) return
  loading.deleting = true
  try {
    ...
    clearDeletePreview()
    if (!isStepUpCancelled(error)) appStore.showError(errorMessage(error, 'admin.promptAudit.errors.deleteConfirmation'))
  } finally { loading.deleting = false }
}
