...
const baseConfig = (): PromptAuditConfig => ({
  enabled: true, blocking_enabled: false, store_pass_events: false, store_full_prompt: false, effective_mode: 'async_audit', strategy: 'priority',
...
describe('PromptAuditView', () => {
  ...
  it('refuses filter deletion until a manual preview has been completed', async () => {
    const wrapper = mountView()
    await flushPromises()
