...
// Destructive filtered deletion is a two-step flow: the administrator must
// explicitly run preview, inspect the matched count/snapshot, then confirm in a
// separate action. Any criteria change clears the preview in the parent.
const confirmDisabled = computed(
  () => !canPreview.value || !props.preview || props.previewing || props.deleting || props.preview.matched_count === 0,
)
const confirmDisabledReason = computed(() => {
  ...
  if (!props.preview) return 'admin.promptAudit.events.filterDeleteNeedPreview'
  if (props.preview.matched_count === 0) return 'admin.promptAudit.events.filterDeleteConfirmNoMatches'
  return ''
})
