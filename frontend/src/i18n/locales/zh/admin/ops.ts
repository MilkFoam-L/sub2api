export default {
    ops: {
      ...
      runtime: {
        ...
        metricThresholdsHint: '设置运行监控指标的健康阈值，超出范围时将在仪表盘中标红。',
        slaMinPercent: 'SLA 最低百分比',
        slaMinPercentHint: 'SLA 低于此值时标红（默认：99.5%）。',
        ttftP99MaxMs: 'TTFT P99 最大值（毫秒）',
        ttftP99MaxMsHint: 'TTFT P99 高于此值时标红（默认：500ms）。',
        requestErrorRateMaxPercent: '请求错误率最大值（%）',
        requestErrorRateMaxPercentHint: '请求错误率高于此值时标红（默认：5%）。',
        upstreamErrorRateMaxPercent: '上游错误率最大值（%）',
        upstreamErrorRateMaxPercentHint: '上游错误率高于此值时标红（默认：5%）。',
        silencing: {
          title: '告警静默（维护模式）',
          enabled: '启用静默',
