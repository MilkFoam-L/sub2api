    ...
    settings: {
      ...
      apiKeyAcl: {
        ...
        trustForwardedIpHint:
          '默认关闭。开启后 CF-Connecting-IP、X-Real-IP 或 X-Forwarded-For 会直接接管客户端 IP 解析并覆盖 server.trusted_proxies；关闭后严格使用 server.trusted_proxies 配置的 Gin 可信代理链。仅在源站无法被直接访问且代理会覆盖这些请求头时显式开启。切换会改变现有会话的 IP 指纹。',
      ...
      gatewayForwarding: {
        ...
        openAIImagesResponsesReasoningEffort: 'OpenAI 图片响应推理强度',
        openAIImagesResponsesReasoningEffortHint: '转发 OpenAI 图片生成 Responses 请求时使用的 reasoning.effort。',
      ...
      customMenu: {
        ...
        visibilityAdmin: '管理员',
        openMode: '打开方式',
        openModeHint: '选择在站内嵌入、当前页面跳转或新标签页中打开。',
        openModeEmbed: '站内嵌入',
